...
int compareStates(State8080 *s1, State8080 *s2);
...
int compCC(ConditionCodes *c1,ConditionCodes *c2)
{
    return ((c1->z != c2->z) ||(c1->s != c2->s) ||(c1->p != c2->p) ||(c1->cy != c2->cy) ||(c1->ac != c2->ac) ||(c1->pad
    != c2->pad));
}

int compareStates(State8080 *s1, State8080 *s2)
{
   if ((s1->a != s2->a) ||(s1->b != s2->b) ||(s1->c != s2->c) ||(s1->d != s2->d) ||(s1->e != s2->e) ||(s1->h != s2->h) ||
       (s1->l != s2->l) || (s1->sp != s2->sp) || (s1->pc != s2->pc) || compCC(&s1->cc, &s2->cc) || (s1->int_enable !=
       s2->int_enable)) {
       printf("///////////////State 1://////////////\n");
       printState(s1);
       printf("///////////////State 2://////////////\n");
...
//For testing purposes
int Emulate8080OpOK(State8080* state)
{
	int cycles = 4;
	unsigned char *opcode = &state->memory[state->pc];
...
int main (int argc, char **argv)
{
    ...
    State8080 state2 = StateCreat(buffer);

    while (state.pc < fsize)
    {
        Emulate8080Op(&state);
        printState(&state);
        Emulate8080OpOK(&state2);
        compareStates(&state, &state2);
    }
    return 0;
}
